...
const generateElectionResultsPDF = async (electionId) => {
  try {
    ...
    doc.fontSize(16)
       .font('Helvetica')
       .text(`Election: ${election.name || 'General Election'}`, { align: 'center' });
    
    doc.moveDown();
    
    const electionDate = election.endTime ? new Date(election.endTime).toLocaleDateString() : new Date().toLocaleDateString();
    doc.fontSize(12)
       .text(`Date: ${electionDate}`, { align: 'center' });
    ...
    doc.fontSize(12)
       .font('Helvetica')
       .text(`Total Votes Cast: ${totalVotes}`);
    ...
    for (const [position, data] of Object.entries(results)) {
      ...
      data.candidates.forEach((candidate, index) => {
        const percentage = data.totalVotes > 0 ? ((candidate.voteCount / data.totalVotes) * 100).toFixed(2) : '0.00';
        
        doc.text(`${candidate.firstName} ${candidate.lastName}`, 50, yPosition)
        ...
        if (index === 0 && candidate.voteCount > 0) {
          doc.fontSize(10)
             .text('WINNER', 50 + nameWidth + votesWidth + percentageWidth + 10, yPosition - rowHeight + 5)
             .fontSize(12);
        }
      });
    }
    ...
    doc.fontSize(12)
       .font('Helvetica')
       .text('This document represents the official results of the election.', { align: 'center' });
    
    doc.moveDown();
    
    doc.fontSize(10)
       .text('Generated automatically by the Election System', { align: 'center' });
...
const sendElectionResultsEmail = async (pdfBuffer, election) => {
  try {
    ...
    const recipientEmails = [
      process.env.RESULTS_EMAIL_1 || 'admin1@example.com',
      process.env.RESULTS_EMAIL_2 || 'admin2@example.com',
      process.env.RESULTS_EMAIL_3 || 'admin3@example.com',
      process.env.RESULTS_EMAIL_4 || 'admin4@example.com'
    ...
    const mailOptions = {
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto;">
          ...
          <div style="background-color: #f8f9fa; padding: 20px; border-radius: 8px; margin: 20px 0;">
            ...
            <p><strong>Election:</strong> ${election.name || 'General Election'}</p>
            <p><strong>Status:</strong> ${election.status}</p>
            <p><strong>Start Time:</strong> ${election.startTime ? new Date(election.startTime).toLocaleString() : 'N/A'}</p>
            <p><strong>End Time:</strong> ${election.endTime ? new Date(election.endTime).toLocaleString() : 'N/A'}</p>
      ...
      attachments: [
        {
          filename: `election-results-${new Date().toISOString().split('T')[0]}.pdf`,
          content: pdfBuffer
        }
      ]
    };
