...
const getDeviceInfo = (req) => {
  return {
    ...
    platform: req.get("User-Agent")
      ? req.get("User-Agent").match(/\(([^)]+)\)/)?.[1] || ""
      : "",
    browser: req.get("User-Agent")
      ? req.get("User-Agent").split(" ")[0] || ""
      : "",
  ...
  try {
    ...
    if (election.status === "ongoing" && election.endTime) {
      ...
      const adjustedEndTime = new Date(
        election.endTime.getTime() + pausedDuration
      );
  ...
  try {
    ...
    if (!validStatuses.includes(status)) {
      return res.status(400).json({
        status: "error",
        message:
          "Invalid status. Must be one of: not_started, ongoing, paused, completed",
      });
    }
    ...
    switch (status) {
      case "ongoing":
        ...
        if (election.status === "not_started") {
          ...
          election.pausedDuration =
            (election.pausedDuration || 0) + pauseDuration;
          election.pausedAt = null;
        }
        break;
    ...
    // Log the status change with a valid action value
