...
exports.startElection = async (req, res) => {
  try {
    ...
    else election.startTime = new Date();
...
exports.updateStatus = async (req, res) => {
  try {
    ...
    await ElectionLog.create({
      electionId: election._id,
      action: status === 'ongoing' ? 'resumed' : status === 'paused' ? 'paused' : 'completed',
...
exports.getLogs = async (req, res) => {
  try {
    ...
    const logs = await ElectionLog.find(query).sort({ timestamp: -1 }).limit(200);
...
exports.generateResults = async (req, res) => {
  try {
    ...
    res.status(200).json({ status: 'success', message: 'Results generated', data: { emailInfo: result?.emailInfo ? { messageId: result.emailInfo.messageId } : null } });
  } catch (error) {
    console.error(error);
    res.status(500).json({ status: 'error', message: 'Server error' });
  }
...
exports.checkElectionStatuses = async (io) => {
  try {
    ...
    if (election.status === 'not_started' && election.startTime && election.startTime.getTime() <= now) {
      election.status = 'ongoing';
      changed = true;
    }

    if (election.status === 'ongoing' && election.endTime && election.endTime.getTime() <= now) {
